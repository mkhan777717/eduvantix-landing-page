...
function CustomGoogleButton({ onSuccess, onError, loading }) {
  const googleLogin = useGoogleLogin({
    onSuccess,
    onError: (err) => {
      console.error("[GOOGLE_AUTH] Popup error:", err);
      onError();
    },
  });
  ...
  return (
    <button
      ...
      onClick={() => googleLogin()}
      disabled={loading}
      className="w-full relative group flex items-center justify-center gap-3 py-3 px-4 rounded-xl border text-sm font-semibold transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:border-[#10b981]/40 hover:shadow-[0_0_20px_rgba(16,185,129,0.08)] active:scale-[0.99] shadow-sm"
      style={{
    ...
    >
      ...
      <span className="tracking-tight text-[13px] font-medium group-hover:text-[var(--text-primary)]">
...
function GoogleLoginSection({ onSuccess, onError, loading, setErrorMsg }) {
  ...
  if (!googleClientId) {
    return (
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border text-sm font-semibold transition-all cursor-pointer disabled:opacity-50 opacity-80 hover:opacity-100 shadow-xs"
        style={{
          backgroundColor: "var(--bg-card)",
          borderColor: "var(--border-primary)",
          color: "var(--text-primary)",
  ...
  return (
    <GoogleOAuthProvider clientId={googleClientId}>
      <CustomGoogleButton onSuccess={onSuccess} onError={onError} loading={loading} />
    </GoogleOAuthProvider>
  );
}
...
function LoginForm() {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    setErrorMsg("");
    try {
      const tokenOrCredential = credentialResponse.credential || credentialResponse.access_token || credentialResponse;
      const result = await loginWithGoogle(tokenOrCredential, referralCode.trim() || undefined);
      if (result.success) {
        let targetRoute;
        if (freeCoursePath) {
